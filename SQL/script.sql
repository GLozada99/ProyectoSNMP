...
CREATE OR REPLACE PROCEDURE get_log(
    IN cantidadi INTEGER
)
BEGIN
    SELECT * FROM (SELECT sl.ID, sl.IP, sl.INTERFACE, sl.DOWNTIME, sl.UPTIME FROM status_log AS sl INNER JOIN interfaces AS i ON i.IP = sl.IP AND i.INTERFACE = sl.INTERFACE WHERE i.ACTIVE = 1 ORDER BY id DESC LIMIT cantidadi) as final ORDER BY id ASC;
END;
|;
